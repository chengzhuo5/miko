...
原始项目中常见 `createWebHashHistory()`——这在 SSG 下不工作。迁移后优先改为 HTML5 history。如业务强依赖 hash（如银行 APP 内嵌），明确切换为 SPA：
...
const routes = [
  {
    path: '/',
    name: 'market',
    component: () => import('@/pages/hq/index.vue'),
    meta: { title: '行情' },
  },
  {
    path: '/stock-detail',
    name: 'stock-detail',
    component: () => import('@/pages/stock-detail/index.vue'),
    meta: { title: '个股详情' },
  },
