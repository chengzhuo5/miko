...
export function createRuntimeModule(options: RuntimeModuleOptions): string {
  const lines = [
    ...
    '  // 自动 ready：首个真实渲染（#app 内出现内容）即标记，无需业务显式调用 markMikoReady。',
    '  // 不依赖路由导航成功/等待首次导航——慢路由、异步组件或导航失败都不应阻塞',
    '  // 白屏解除（零配置）；挂载但无内容（如无匹配路由的 RouterView）不标记，',
    '  // 留给白屏监控在超时后接管。mounted 仅客户端执行，SSR 无 window 时是 no-op。',
    '  app.mixin({',
    '    mounted() {',
    '      if (getPendingMikoBoot()) {',
    "        const root = typeof document === 'undefined' ? undefined : document.getElementById('app')",
